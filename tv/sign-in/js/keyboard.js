...
Main.login = function(name,password)
{
	...
	var url='http://api.majitv.com/api/member_tv/login.ashx';
	var dataString= 'usr=' + name + '&pwd=' + password;
	var LoginURL='http://api.majitv.com/api/member_tv/login.ashx?usr=' + name + '&pwd=' + password;
	console.log("login");
    ...
    if (Main.XHRObj)
    {
                ...
                if (Main.XHRObj.readyState == 4)
                {
					...
					else
					{
						...
						if(xmlElement["status"]){
							...
							Main.username = name;
							Main.pwd = password;
							window.name = "true|" + name + "|" + password;
		...
		var tmpURL = 'http://api.majitv.com/api/member_tv/login.ashx?' + 	dataString;
