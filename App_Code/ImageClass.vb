...
Public Class ImageClass
    ...
    Public Shared Function AddWatermarkImage(ByVal OriginalImage As Image, ByVal watermark_black_path As String, ByVal watermark_white_path As String) As Image



        Dim imgWatermark As Image = New Bitmap(Bitmap.FromFile(watermark_white_path))

        Dim imgWatermark_width As Integer = imgWatermark.Width
        Dim imgWatermark_height As Integer = imgWatermark.Height


        Dim imgWidth As Integer = OriginalImage.Width
        Dim imgHeight As Integer = OriginalImage.Height
        'Dim watermark_point = New Point(0, 0)
        Dim watermark_point As Point

        Dim watermark_from_point_x = imgWidth * 0.35
        Dim watermark_from_point_y = imgHeight * 0.9

        Dim check_to_point_x As Integer = watermark_from_point_x + imgWatermark_width
        Dim check_to_point_y As Integer = watermark_from_point_y + imgWatermark_height

        watermark_point = New Point(watermark_from_point_x, watermark_from_point_y)

        Dim black_color_count As Integer = 0
        Dim white_color_count As Integer = 0

        Dim bmp As Bitmap = OriginalImage
        For x As Integer = watermark_from_point_x To check_to_point_x
            For y As Integer = watermark_from_point_y To check_to_point_y
                ' do your logic for count the color instances... 
                Dim thisColor As Color = bmp.GetPixel(x, y)

                'RGB black(0,0,0) white(255,255,255)
                If thisColor.R > 127 And thisColor.G > 127 And thisColor.B > 127 Then
                    white_color_count += 1
                Else
                    black_color_count += 1
                End If
        ...
        'user white water mark if more black color found
        If black_color_count > white_color_count Then

            imgWatermark = New Bitmap(Bitmap.FromFile(watermark_white_path))
        Else
            imgWatermark = New Bitmap(Bitmap.FromFile(watermark_black_path))
        ...
        g.CompositingQuality = Drawing2D.CompositingQuality.HighQuality
        g.InterpolationMode = Drawing2D.InterpolationMode.High
        g.DrawImage(imgWatermark, watermark_point)
        Dim img As New Bitmap(OriginalImage)
        g.DrawImage(img, New Point(0, 0))
        Return img
    End Function
