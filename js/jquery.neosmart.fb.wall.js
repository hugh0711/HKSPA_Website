        ...
        return meta.each(function () {
            ...
            ******************************************************************************************************/
            ...
            var initWall = function (data) {
                ...
                // Add Post Box
                if (accessToken != '') {
                    output += addStatus(o.id, fbId, fbName);
                };
                ...
                for (var k = 0; k < max; k++) {
                    ...
                    // Box -----------------------------------------------------------------------------------------------------------------------------------
                    output += (k == 0 && accessToken == '') ? '<div id="fb-' + data[k].id + '" class="fb-wall-box fb-wall-box-first">' : '<div id="fb-' + data[k].id + '"class="fb-wall-box">';
                    ...
                    // Question ----------------------------------------------------------------------------------------------------------------------------------
                    if (data[k].type == "question") {
                        var qid = data[k].object_id;
                        output += '<div class="fb-wall-poll" id="fb-q-' + qid + '" class="loading"></div>';
                        loadQuestion(qid);
                    }
                    else {
                        // Normal Message ---------------------------------------------------------
                        ...
                        if (exists(data[k].story)) output += '<br />' + modText(data[k].story);
                        output += '</span>';
                    }
                    ...
                    if (exists(data[k].picture) || exists(data[k].link) || exists(data[k].caption) || exists(data[k].description)) {
                        ...
                        if (data[k].type == "photo") {
                            if (exists(data[k].link)) output += '<a href="' + data[k].link + '" target="_blank" class="fb-wall-media-link">';
                            output += '<img class="fb-wall-picture" src="https://graph.facebook.com/' + data[k].object_id + '/picture?type=album" />';
                        ...
                        output += '</div>'; // class="fb-wall-media-container"
                        output += '</div>'; // class="fb-wall-media"
                    }
                    ...
                    if (exists(data[k].comments) && exists(data[k].comments.data) && (o.showComments == true || o.showComments == 'true')) {
                        ...
                        if (accessToken != '') {
                            output += '<span class="fb-wall-comment" id="fb-post-' + data[k].id + '">';
                            output += '<textarea class="watermark">' + replyWatermarkText + '</textarea>';
                        ...
                        if (data[k].type != "question" && accessToken != '') {
                            output += '<span class="fb-wall-comment" id="fb-post-' + data[k].id + '">';
                            output += '<textarea class="watermark">' + replyWatermarkText + '</textarea>';
                            output += '</span>';
                        };
                ...
                //$(".fb-wall-comment textarea").autoSize();
            }
            ...
            ******************************************************************************************************/
            ...
            function loadQuestion(id) {
                var url = graphURL + id + '?access_token=' + opts.accessToken;
                $.ajax({
                    url: url,
                    dataType: "jsonp",
                    success: function (data) {
                        ...
                        var output = '';
                        output += '<span class="fb-wall-message">';
                        output += '<a href="http://www.facebook.com/profile.php?id=' + data.from.id + '" class="fb-wall-message-from" target="_blank">' + data.from.name + '</a> ';
                        if (exists(data.question)) output += ' asked: ' + '<a href="http://www.facebook.com/questions/' + id + '" target="_blank" >' + modText(data.question) + '</a>';
                        output += '</span>';

                        output += '<table class="fb-wall-poll-table" cellpadding="1" cellspacing="0">';
                        var votes = 0;
                        $.each(data.options.data, function (index, value) {
                            votes += value.votes;
                        });
                        $.each(data.options.data, function (index, value) {
                            output += '<tr>';
                            // Column 1
                            output += '<td class="pollRadioBtn">';
                            output += '</td>';
                            // Column 2
                            output += '<td class="pollResultsBar">';
                            output += '<div class="pas fbQuestionsPollResultsBar" title="' + value.name + '">';
                            output += '<div class="shaded" style="width:' + Math.round(value.votes * 100 / votes) + '%"></div>';
                            output += '<div class="label"><span class="fwb">' + value.name + '</span></div>';
                            output += '<div class="fbQuestionsPollClickTarget"></div>';
                            output += '<div class="phs auxlabel"><span class="fwb fcg">' + value.votes + (value.votes == 1 ? ' vote' : ' votes') + '</span></div>';
                            output += '</div>';
                            output += '</td>';
                            output += '</tr>';
                        });
                        output += '</table>';
                        $("#fb-q-" + id).html(output).removeClass('loading');
                    },
...
function addStatus(pageId, fbId, fbName) {
    var output = '';
    output += '<div class="fb-wall-box fb-wall-box-first">';
    output += '<img class="fb-wall-avatar" src="https://graph.facebook.com/' + fbId + '/picture?type=square" />';
    output += '<div class="fb-wall-data">';
    output += '<span class="fb-wall-message"><span class="fb-wall-message-from">' + fbName + '</span></span>';

    //output += '<span class="fb-wall-status" id="fb-post-' + o.id + '">';
    output += '<textarea class="watermark fb-wall-status" id="fb-status-' + pageId + '">' + replyWatermarkText + '</textarea>';
    //output += '</span>';
