...
Partial Class backoffice_product_upload
    ...
    Protected Sub btnCrop_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCrop.Click
        Dim ImageAdapter As New ProductDataSetTableAdapters.ProductImageTableAdapter()
        Dim url As String = Path.GetFileNameWithoutExtension(ImageAdapter.GetImageUrl(CInt(hfdProductID.Value), 1)) & ".png"
        Session("WorkingImage") = url
        Dim UploadPath As String = Path.Combine(ConfigurationManager.AppSettings("UploadPath"), url)
        Dim OriginalPath As String = Path.Combine(ConfigurationManager.AppSettings("ProductOriginalImagePath"), url)
