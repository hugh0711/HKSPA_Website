    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        If txtProductName.Text.Length > 0 Then
            If ReorderList1.DataSourceID = "SqlDataSourceByCategory" Then
                SqlDataSourceByCategory.SelectCommand = "SELECT ProductID, ProductCode, ProductName, CategoryName, Url, Description, Enabled, SortOrder, ThumbnailWidth, ThumbnailHeight FROM view_ProductImage WHERE (CategoryID = @CategoryID) AND (Lang = @Lang) AND (FunctionID = @FunctionID) AND ProductName LIKE '%" & txtProductName.Text & "%' ORDER BY CategoryID, SortOrder"
            Else
                SqlDataSourceAll.SelectCommand = "SELECT ProductID, ProductCode, ProductName, CategoryName, Url, Enabled, SortOrder, ThumbnailWidth, ThumbnailHeight, Description FROM view_ProductImage WHERE (Lang = @Lang) AND (FunctionID = @FunctionID) AND ProductName LIKE '%" & txtProductName.Text & "%' ORDER BY CategoryID, SortOrder"
            End If
            ReorderList1.AllowReorder = False
            ReorderList1.DataBind()
        End If
