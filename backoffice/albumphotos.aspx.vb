...
Partial Class backoffice_albumphotos
    ...
    Protected Sub AjaxFileUpload1_UploadComplete(sender As Object, e As AjaxControlToolkit.AjaxFileUploadEventArgs) 'Handles AjaxFileUpload1.UploadComplete
        ...
        Try
			Dim Watermark_Path As String = ConfigurationManager.AppSettings("WatermarkPath")
			Dim Watermark_Back As String = ConfigurationManager.AppSettings("Watermark_Black")
			Dim Watermark_White As String = ConfigurationManager.AppSettings("Watermark_White")
			Dim Watermark_Back_tb As String = ConfigurationManager.AppSettings("Watermark_Black_tb")
			Dim Watermark_White_tb As String = ConfigurationManager.AppSettings("Watermark_White_tb")
			Dim input_watermark_black_path As String = MapPath(IO.Path.Combine(Watermark_Path, Watermark_Back))
			Dim input_watermark_white_path As String = MapPath(IO.Path.Combine(Watermark_Path, Watermark_White))
			Dim input_watermark_tb_path As String = MapPath(IO.Path.Combine(Watermark_Path, Watermark_Back_tb))
			...
			Dim original_filepath As String = Path.Combine(AlbumPath, Filename)
			...
			If chk_waterMark.Checked Then
				Image1 = ImageClass.AddWatermarkImage(Image1, input_watermark_black_path, input_watermark_white_path)
			End If
			...
			'Image2 = ImageClass.AddWatermarkImage(Image2, input_watermark_path)
			ImageClass.SaveJPGWithCompressionSetting(Image2, MapPath(FilePath), JpegCompression)
			Image2.Dispose()
			Image.Dispose()

			Dim input_fileName As String = Path.GetFileNameWithoutExtension(e.FileName)

			'find original file name and author name and split, example: 1_HughYu
			Dim o_filename = input_fileName.Split("_")
			...
			'UploadPhotoToFB(PhotoID, Session("tokens"), MapPath(original_filepath))
		...
		Catch ex As Exception
            ...
            errorMessage &= String.Format("{0} {1}", ex.Message.ToString(), Environment.NewLine)
        End Try
    ...
    Protected Sub UploadNewsToFB(tokens As Dictionary(Of String, String), product_ID As Integer)
        ...
        Dim image_URL As String = String.Format("http://hkspa.innowil.com/product_image/product/{0}", product_ID)

        client.Post("me/feed", New With { _
        Key .Caption = "Image Share", _
        Key .Description = "Test Desc", _
        Key .ImageUrl = image_URL, _
        Key .message = "My first wall post using Facebook SDK for .NET", _
        Key .Name = "Test Post", _
        Key .Url = "http://blog.impact-works.com"
        })
    ...
    Protected Sub UploadPhotoToFB(PhotoID As Guid, tokens As Dictionary(Of String, String), pathToFiles As String)
        ...
        Dim return_photoID As String = ""

        If exist = True Then
            return_facebook_photo_ID = UploadPhotoToFB(client, FB_result.Item("id").ToString, pathToFiles, pathToFiles, photo_info.Author)
        ...
        'update Facebook_photoID
        Dim album_detail = (From a In db.AlbumPhoto_infos
                         Where a.PhotoID = PhotoID
                         Select a).FirstOrDefault()

        If album_detail IsNot Nothing And return_facebook_photo_ID.Length > 0 Then
            album_detail.Facebook_photoID = return_facebook_photo_ID
    ...
    Protected Function UploadPhotoToFB(client As FacebookClient, AlbumID As String, image_name As String, image_path As String, Author As String) As String
        ...
        'return_photoID = client.Post(String.Format("/{0}/photos", AlbumID), upload)
        Dim the_return_photoID As Facebook.JsonObject = client.Post(String.Format("/{0}/photos", AlbumID), upload)

        return_photoID = the_return_photoID.Item("id").ToString()


        Return return_photoID
    End Function
    ...
    Protected Sub InsertPhotoInfo(PhotoID As Guid, image_info As Goheer.EXIF.EXIFextractor, authorName As String)
        ...
        Try
            aperture = image_info.Item("Aperture").ToString
        Catch ex As Exception
            aperture = ""
        End Try

        Try
            shutter = image_info.Item("Shutter Speed").ToString
        Catch ex As Exception
            shutter = ""
        End Try
        ...
        Dim ap_info As New AlbumPhoto_info With {
            ...
            .f_stop = If(f_stop.Length > 0, f_stop, ""),
            ...
            .Author = authorName,
            .aperture = If(aperture.Length > 0, aperture, ""),
            .shutter = If(shutter.Length > 0, shutter, "")
        }
