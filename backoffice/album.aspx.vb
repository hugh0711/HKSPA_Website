...
Partial Class backoffice_album
    ...
    Protected Sub CreateTab()
        ...
        For i As Integer = 0 To Desc.Count - 1
            ...
            Dim AlbumLangControl As UserControl = LoadControl("~/control/AlbumLangControl.ascx")
            With CType(AlbumLangControl, control_AlbumLangControl)
                .ID = String.Format("AlbumLangControl{0}", i + 1)
                .SetLang(Lang(i))
            End With
            Tab.Controls.Add(AlbumLangControl)
    ...
    Protected Sub LoadTab()
        Dim Lang() = ConfigurationManager.AppSettings("LanguageSupport").Split(",")
        Dim AlbumID As Guid = Guid.Parse(Request("album"))

        For i As Integer = 0 To TabContainer1.Tabs.Count - 1
            Dim Tab As AjaxControlToolkit.TabPanel = TabContainer1.Tabs(i)
            Dim c As UserControl = Tab.FindControl(String.Format("AlbumLangControl{0}", i + 1))
            With CType(c, control_AlbumLangControl)
                .LoadAlbumLang(AlbumID, Lang(i))
            End With
        Next
    ...
    Protected Sub SaveTab(AlbumID As Guid)

        For i As Integer = 0 To TabContainer1.Tabs.Count - 1
            Dim Tab As AjaxControlToolkit.TabPanel = TabContainer1.Tabs(i)
            Dim c As UserControl = Tab.FindControl(String.Format("AlbumLangControl{0}", i + 1))
            CType(c, control_AlbumLangControl).SaveAlbumLang(AlbumID)
        Next
    End Sub
